...
exports.register = asyncHandler(async (req, res, next) => {
  const { name,email,password,Daddress,phoneNo } = req.body;
  const user = await User.create({
  name, email, phoneNo, password, Daddress
  });
...
exports.login = asyncHandler(async (req, res, next) => {
  ...
  if (!phoneNo || !password) {
    return next(new ErrorResponse("Please provide phoneNo and password"), 400);
  }
